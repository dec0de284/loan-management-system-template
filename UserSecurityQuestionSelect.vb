...
Public Class UserSecurityQuestionSelect
    ...
    Private Sub btn_Edit_Click(sender As Object, e As EventArgs) Handles btn_Edit.Click
        ...
        If UserAccountSecurityQuestions.pressedButton = 0 Then
            For Each item As String In UserAccountSecurityQuestions.SelectedQuestions
            ...
            Else
                UserAccountSecurityQuestions.TextBox1.Text = SelectedQuestion
                UserAccountSecurityQuestions.SelectedQuestions(0) = SelectedQuestionID
                ErrorOccured = False
            End If
        ElseIf UserAccountSecurityQuestions.pressedButton = 1 Then
            For Each item As String In UserAccountSecurityQuestions.SelectedQuestions
                If item = SelectedQuestionID Then
                    ErrorOccured = True
                End If
            ...
            If ErrorOccured Then
                InformationMessage("The question is already selected.", "Duplicate Security Question")
            Else
                UserAccountSecurityQuestions.TextBox4.Text = SelectedQuestion
                UserAccountSecurityQuestions.SelectedQuestions(1) = SelectedQuestionID
                ErrorOccured = False
            End If
        ElseIf UserAccountSecurityQuestions.pressedButton = 2 Then
            For Each item As String In UserAccountSecurityQuestions.SelectedQuestions
                If item = SelectedQuestionID Then
                    ErrorOccured = True
                End If
            ...
            If ErrorOccured Then
                InformationMessage("The question is already selected.", "Duplicate Security Question")
            Else
                UserAccountSecurityQuestions.TextBox6.Text = SelectedQuestion
                UserAccountSecurityQuestions.SelectedQuestions(2) = SelectedQuestionID
                ErrorOccured = False
            End If
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If validQuestion.IsMatch(TextBox1.Text) And TextBox1.Text.Length > 9 Then
            If MessageBox.Show("Add '" + TextBox1.Text + "' to question list?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                ...
                Database.Insert("INSERT INTO [dtb_AuditLog] ([LogID], [ActionBy], [Action], [ActionDate], [ActionState], [ActionLevel]) VALUES(" + (LogID + 1).ToString + ",'" + AccountType.GetAccountUsername() + "','Add Security Question','" + Date.Now.ToString() + "','Success','Information');", False)
                Database.PopulateDataGrid("SELECT * FROM [dtb_SecurityQuestions] ORDER BY [ID] DESC;", SystemAccountsTable, New DataGridViewAutoSizeColumnMode() {DataGridViewAutoSizeColumnMode.DisplayedCells, DataGridViewAutoSizeColumnMode.Fill})
