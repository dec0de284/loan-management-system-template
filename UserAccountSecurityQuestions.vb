...
Public Class UserAccountSecurityQuestions
        ...
        Else
            Try
                ...
                If Database.Count("SELECT COUNT([AccountID]) FROM [dtb_SystemAccounts];") = 0 Then
                    AccountID = 0
                Else
                    AccountID = Database.ReturnLastID("AccountID", "dtb_SystemAccounts")
                End If
                Database.Insert("INSERT INTO [dtb_SystemAccounts] ([AccountID],[RoleID],[Username],[Password],[Active],[DateCreated],[CreatedBy],[QID1],[A1],[QID2],[A2],[QID3],[A3]) VALUES(" + (AccountID + 1).ToString + ",'0','" + UserAccountCreation.newUserUsername + "','" + UserAccountCreation.newUserPassword + "', '1','" + Date.Now.ToString() + "','" + AccountType.GetAccountUsername() + "','" + SelectedQuestions(0) + "','" + TextBox2.Text + "','" + SelectedQuestions(1) + "','" + TextBox3.Text + "','" + SelectedQuestions(2) + "','" + TextBox5.Text + "');", True)
                ...
                Database.Insert("INSERT INTO [dtb_AuditLog] ([LogID], [ActionBy], [Action], [ActionDate], [ActionState], [ActionLevel]) VALUES(" + (LogID + 1).ToString + ",'" + AccountType.GetAccountUsername() + "','Create User Account','" + Date.Now.ToString() + "','Success','Information');", False)
                Me.Close()
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        pressedButton = 0
        UserSecurityQuestionSelect.Show()
